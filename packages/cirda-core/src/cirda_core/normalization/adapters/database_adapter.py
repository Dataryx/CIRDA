"""Database audit evidence adapter."""
...
class DatabaseAdapter:
    """Normalize database audit log records."""

    channel = EvidenceChannel.DATABASE

    def can_handle(self, raw: dict[str, Any]) -> bool:
        return raw.get("source") == "database" or raw.get("channel") == self.channel.value
    ...
    def normalize(self, raw: dict[str, Any]) -> EvidenceEvent:
        ...
        if not isinstance(observed_at, datetime):
            raise ValueError("database adapter requires observed_at datetime")
        return build_event(
            event_id=str(raw["event_id"]),
            source_id=str(raw["actor_id"]),
